# CLAUDE.md — Project Instructions & Agent Definitions

Source of truth for this repo. Claude Code, Codex, and other AI tools should
read this file for all project context.
...
All Claude Code skills live in `.claude/skills/` and should be treated as
first-class project capabilities.

Before starting a task:

1. Scan `.claude/skills/` for the closest matching skill.
2. Read the selected skill's `SKILL.md`.
3. Follow its Purpose, When to use, Inputs, Outputs, Steps, Examples, and Guardrails.
4. If multiple skills apply, use the smallest useful combination and state the order.
5. If no active skill fits, use normal project conventions and consider whether a new skill belongs in `.claude/skills/`.

Skill priority:

- Prefer active project skills at `.claude/skills/<kebab-case>/SKILL.md`.
- Use `.claude/skills/lenny-podcast/<skill>/SKILL.md` when a Lenny PM skill is the best fit.
- Treat `todo-` skills as placeholders unless their `SKILL.md` contains enough implementation detail to execute safely.
...
- CLAUDE.md is the source of truth. AGENTS.md points agents here.
